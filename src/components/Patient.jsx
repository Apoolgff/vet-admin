const Patient = ({patient, setPatient, deletePatient}) => {
   const {name, owner, email, date, symptoms, id} = patient
    ...
    return ( 
        <div className="mx-5 my-10 bg-green-50 shadow-md px-5 py-10 rounded-xl border-solid border-2 border-black">
            <p className="font-bold mb-3 text-gray-700 uppercase">Nombre: {''}
            <span className="font-normal normal-case">{name}</span></p>

            <p className="font-bold mb-3 text-gray-700 uppercase">Propietario: {''}
            <span className="font-normal normal-case">{owner}</span></p>
            ...
            <span className="font-normal normal-case">{symptoms}</span></p>
